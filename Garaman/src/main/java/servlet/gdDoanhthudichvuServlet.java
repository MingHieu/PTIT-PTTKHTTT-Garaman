...
@WebServlet("/doanhthudichvu")

public class gdDoanhthudichvuServlet extends HttpServlet {
    @Override
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        DoanhthuRes doanhthuRes = Dichvu248DAO.instance().getDoanhThu(pageNumber, from, to);
        doanhthuRes.setPageNumber(pageNumber);

        req.setAttribute("doanhthuRes", doanhthuRes);
        req.getRequestDispatcher("/view/ql/gdDoanhthudichvu.jsp").forward(req, resp);
    }
}