...
<t:main title="Doanh thu phụ tùng" role="QL" js="/js/gdDoanhthuphutung.js">
    <jsp:body>
        <div class="w-fit min-w-[50%] mx-auto mb-5">
            ...
            <div date-rangepicker class="flex items-center my-5">
                ...
                <button type="button"
                        class="xem-TK text-white bg-gradient-to-br from-purple-600 to-blue-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center ml-5 ">
                    Xem
                </button>
            </div>
                    ...
                    <c:forEach var="i" begin="0" end="${doanhthuList.size() - 1}">
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                            <th scope="row"
                                ...
                                <a href="/phutungsudung?id=${doanhthuList.get(i).getId()}">${(pageSize * (pageNumber - 1)) + i + 1}</a>
                            </th>
                            <td class="px-6 py-4">
                                <a href="/phutungsudung?id=${doanhthuList.get(i).getId()}">PT${doanhthuList.get(i).getId()}</a>
                            </td>
                            <td class="px-6 py-4">
                                <a href="/phutungsudung?id=${doanhthuList.get(i).getId()}">${doanhthuList.get(i).getTen()}</a>
                            </td>
                            <td class="px-6 py-4 text-right">
                                <a href="/phutungsudung?id=${doanhthuList.get(i).getId()}">${doanhthuList.get(i).getDoanhthu()}</a>
                            </td>
                        </tr>
                    </c:forEach>
                </c:if>
