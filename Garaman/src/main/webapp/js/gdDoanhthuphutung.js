...
const phutungLinks = useSelectorAll("[href^='/phutungsudung']")
phutungLinks.forEach(phutung => {
    phutung.onclick = (e) => {
        e.preventDefault()
        const id = new URL(phutung.href).searchParams.get("id")
        const queryURL = new URLSearchParams(location.search)
        queryURL.set("id", id)
        queryURL.delete("page")
        const url = new URL(location.origin + "/phutungsudung")
        url.search = queryURL.toString()
        location.href = url
    }
})